...
use serde::{ser::{Serialize, SerializeTuple}, Deserialize, de::{Visitor, self}};
...
impl<'de> Deserialize<'de> for SubMessageHeader {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
        ...
        impl<'de> Visitor<'de> for SubMessageHeaderVistor {
            ...
            {
                let id: u8 = seq.next_element()?
                        .ok_or_else(|| de::Error::invalid_length(0, &self))?;
                let flag: u8 = seq.next_element()?
                    .ok_or_else(|| de::Error::invalid_length(1, &self))?;
                let len: u16 = seq.next_element()?
                ...
                let result = match id {
                        ...
                        5 => Status(len),
                        6 => Info(len),
                        7 => WriteData(len, DataFormat::try_from(flag >> 1).map_err(|e| 
                            match e {
                                crate::error::Error::InvalidFormat(v) => de::Error::invalid_value(de::Unexpected::Unsigned(v as u64), &self),
                                _ => unreachable!()
                            })?
                        ),
                        8 => ReadData(len),
                        9 => Data(len, DataFormat::try_from(flag >> 1).map_err(|e| 
                            match e {
                                crate::error::Error::InvalidFormat(v) => de::Error::invalid_value(de::Unexpected::Unsigned(v as u64), &self),
                                _ => unreachable!()
                            })?),
