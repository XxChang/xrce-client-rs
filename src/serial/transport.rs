...
impl<Comm: SerialPlatformOps> SerialTransport<Comm> {
    ...
    fn framing_write_transport(&mut self) -> Result<usize> {
        ...
        loop {
            let last_written = self.platform.write_serial_data(&self.framing_io.wb[bytes_written..(self.framing_io.wb_pos - bytes_written)])?;
    ...
    fn get_next_octet(&mut self) -> Option<u8>
    {
        if self.framing_io.rb_head != self.framing_io.rb_tail {
            if FRAMING_ESC_FLAG != self.framing_io.rb[self.framing_io.rb_tail]
            {
                ...
                self.framing_io.rb_tail = (self.framing_io.rb_tail + 1) % core::mem::size_of_val(&self.framing_io.rb);
                ...
                let temp_tail = (self.framing_io.rb_tail + 1) % core::mem::size_of_val(&self.framing_io.rb);
                if temp_tail != self.framing_io.rb_head
                {
                    let mut octet = self.framing_io.rb[temp_tail];
                    self.framing_io.rb_tail = (self.framing_io.rb_tail + 2) % core::mem::size_of_val(&self.framing_io.rb);
                    if octet != FRAMING_BEGIN_FLAG
                    {
    ...
    fn write_framed_msg(&mut self, buf: &[u8], remote_addr: u8) -> Result<usize>
    {
        ...
        while written_len < buf.len()
        {
            ...
            if self.add_nex_octet(octet)
            {
                ...
                self.framing_write_transport().map_err(|e| {
                    match e {
    ...
    fn framing_read_transport(&mut self, timeout: &mut i32, max_size: usize) -> Result<usize>
    {
        ...
        if self.framing_io.rb_head == self.framing_io.rb_tail {
            ...
            (
                self.framing_io.rb_tail - self.framing_io.rb_head - 1,
                0
            )
        };
        ...
        if 0 < av_len.0
        {
            ...
            // !todo
            bytes_read[0] = self.platform.read_serial_data(&mut self.framing_io.rb[self.framing_io.rb_head..], av_len.0, timeout_temp)?;
            self.framing_io.rb_head = (self.framing_io.rb_head + bytes_read[0]) % core::mem::size_of_val(&self.framing_io.rb);
            if 0 < bytes_read[0]
            {
                if (bytes_read[0] == av_len.0) && (0 < av_len.1)
                {
                    bytes_read[1] = self.platform.read_serial_data(&mut self.framing_io.rb[self.framing_io.rb_head..], av_len.1, 0)?;
                    self.framing_io.rb_head = (self.framing_io.rb_head + bytes_read[1]) % core::mem::size_of_val(&self.framing_io.rb);
                }
            }
        }
        ...
        if self.framing_io.rb_tail != self.framing_io.rb_head
        {
            'outer: loop {
                match self.framing_io.state {
                    FramingUninitialized => {
                        let mut octet = 0;
                        while (FRAMING_BEGIN_FLAG != octet) && (self.framing_io.rb_head != self.framing_io.rb_tail) 
                        {
