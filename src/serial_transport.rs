      ...
      RX: serial::Read<u8>,
{
    ...
    fn write_framed_msg(&mut self, buf: &[u8], remote_addr: u8) -> nb::Result<(), ()> {
        block!(self.tx.write(FRAMING_BEGIN_FLAG)).ok();
        
        block!(self.tx.write(self.local_addr)).ok();
        block!(self.tx.write(remote_addr)).ok();
        let len = buf.len().to_le_bytes();
        block!(self.tx.write(len[0])).ok();
        block!(self.tx.write(len[1])).ok();

        let mut crc: u16 = 0;

        for byte in buf {
            block!(self.tx.write(*byte)).ok();
            uxr_update_crc(&mut crc, *byte);
        }

        let crc_bytes = crc.to_le_bytes();
        block!(self.tx.write(crc_bytes[0])).ok();
        block!(self.tx.write(crc_bytes[1])).ok();
        Ok(())
    }
}

impl<TX, RX> Transmitter for SerialTransport<TX, RX>
where TX: serial::Write<u8>,
      RX: serial::Read<u8>,
{
    type Ok = ();
    ...
    fn send_msg(&mut self, buf: &[u8]) -> core::result::Result<Self::Ok, Self::Error> {
        match self.write_framed_msg(buf, self.remote_addr) {
            Ok(_) => Ok(()),
            Err(_) => Err(()),
        }
    }
